...
// Simulation class (moved before Equation to ensure complete type)
class Simulation {
private:
...
public:
    int size;
    long double dx, dt, t;
    std::vector<std::unique_ptr<class Equation>> equations;
    ...
    Simulation(int grid_size = 64, long double delta_x = 0.1L, long double delta_t = 0.01L)
        : size(grid_size), dx(delta_x), dt(delta_t), t(0.0L) {
        ...
        scalar_4d.resize(size, std::vector<std::vector<std::vector<long double>>>(size,
            std::vector<std::vector<long double>>(size, std::vector<long double>(size, 0.0L))));
        particles.resize(100);
        logger.log("Simulation initialized with grid size " + std::to_string(size) + ", dx=" + std::to_string(dx) + ", dt=" + std::to_string(dt));
    }

    void load_equations() {
        equations.clear();
        equations.push_back(std::make_unique<CustomEquation>(
            "gravity",
            "G*mass1*mass2/r^2",
            std::map<std::string, long double>{{"G", 6.67430e-11L}, {"mass1", 5.972e24L}, {"mass2", 7.348e22L}, {"r", 3.844e8L}},
            std::map<std::string, std::string>{{"force", "G*mass1*mass2/r^2"}},
            true
        ));
        equations.push_back(std::make_unique<TensorEquation>(
            "tensor_field",
            "sum(H,eps,L0)",
            std::map<std::string, long double>{{"H", 1.0e-18L}, {"eps", 0.1L}, {"L0", 1.0L}, {"scale", 1.4796L}},
            std::map<std::string, std::string>{}
        ));
        equations.push_back(std::make_unique<QuantumEquation>(
            "hydrogen_1s",
            std::map<std::string, long double>{{"hbar", 1.05457182e-34L}, {"m", 9.1093837e-31L}, {"a0", 5.29e-11L}}
        ));
        equations.push_back(std::make_unique<OriginalEquation>(
            "original",
            std::map<std::string, long double>{{"H1D", 1.0L}, {"D_t", 0.1L}, {"eps_5D", 0.1L}}
        ));
        equations.push_back(std::make_unique<RefinedEquation>(
            "refined",
            std::map<std::string, long double>{{"phi", 1.0L}, {"G", 1.0L}, {"k4", 0.1L}, {"k5", 0.01L}}
        ));
    ...
    void run(int steps) {
        for (int step = 0; step < steps; ++step) {
            for (const auto& eq : equations) {
                if (eq->is_valid()) {
                    eq->compute(*this);
                    std::ofstream out(eq->name() + "_output_" + std::to_string(step) + ".csv");
                    if (out.is_open()) {
                        out << eq->output(*this);
                        out.close();
                    }
                    logger.log("Step " + std::to_string(step) + ": Computed and saved output for " + eq->name());
                } else {
                    logger.log("Step " + std::to_string(step) + ": Skipped invalid equation " + eq->name());
                }
            }
        }
        logger.flush();
    ...
    void setVisScale(const std::string& eq_name, long double scale) {
        for (auto& eq : equations) {
            if (eq->name() == eq_name) {
                ...
                logger.log("Set visualization scale for " + eq_name + " to " + std::to_string(scale));
                return;
            }
        }
        logger.log("ERROR: Equation " + eq_name + " not found for setVisScale");
    }
    ...
    void setVisColorIntensity(const std::string& eq_name, long double intensity) {
        for (auto& eq : equations) {
            if (eq->name() == eq_name) {
                ...
                logger.log("Set visualization color intensity for " + eq_name + " to " + std::to_string(intensity));
                return;
            }
        }
        logger.log("ERROR: Equation " + eq_name + " not found for setVisColorIntensity");
    }
...
public:
    ...
    virtual void setVisScale(long double s) {}
    virtual long double getVisScale() const { return 1.0L; }
    virtual void setVisColorIntensity(long double c) {}
    virtual long double getVisColorIntensity() const { return 1.0L; }
};
...
public:
    ...
    auto current_time = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::duration<long double>>(current_time - start_time).count();
    long double phi = phi_base + 0.1L * std::sin(sim.t);
        ...
        for (int i = 0; i < sim.size; ++i) {
            for (int j = 0; j < sim.size; ++j) {
                for (int k = 0; k < sim.size; ++k) {
                    ...
                    long double y = static_cast<long double>(j - sim.size / 2) * sim.dx;
                    long double z = static_cast<long double>(k - sim.size / 2) * sim.dx;
                    long double r = std::sqrt(x * x + y * y + z * z + 1e-10L);

                    std::map<std::string, long double> local_vars = {
                        {"H", 1.0e-18L},
                        {"eps", 0.1L},
                        {"L0", 1.0L},
                        {"scale", 1.4796L},
                        ...
                        {"r", r},
                        {"t", sim.t},
                        {"theta", r > 1e-6L ? std::acos(z / r) : 0.0L},
                        {"phi", std::atan2(y, x)},
                        {"phi_base", phi},
                        {"G", G},
                        {"k4", k4},
                    ...
                    try {
                        // Compute the refined scalar field: phi + G * (k4 * r^2 + k5 * r^4)
                        long double r2 = r * r;
                        long double r4 = r2 * r2;
                        long double value = phi + G * (k4 * r2 + k5 * r4);
                        long double vis_value = value * vis_scale;
                        vis_value = std::tanh(vis_value / vis_color_intensity);
                        ...
                        #pragma omp critical
                        {
                            ...
                            // Compute divergence for inner grid points
                            if (i > 0 && i < sim.size - 1 && j > 0 && j < sim.size - 1 && k > 0 && k < sim.size - 1) {
                                Tensor& div = sim.divergence[i][j][k];
                                for (int nu = 0; nu < 3; ++nu) {
                                    for (int mu = 0; mu < 3; ++mu) {
                        ...
                        logger.log("ERROR: Failed to compute refined equation at (" + std::to_string(i) + "," + 
                                  std::to_string(j) + "," + std::to_string(k) + "): " + e.what());
                    }
                }
            }
        }
    ...
    sim.t += sim.dt;
    logger.log("Computed step for " + eq_name + " at t=" + std::to_string(sim.t) + 
              ", progress=" + std::to_string(std::min(elapsed / duration, 1.0L)));
    logger.flush();
}
