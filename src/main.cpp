...
// Declare init_ttf from renderer.cpp
bool init_ttf();
...
int main() {
    ...
    // Initialize TTF before menu
    if (!init_ttf()) {
        log_message("TTF initialization failed, continuing without menu");
        std::cerr << "TTF initialization failed, continuing without menu" << std::endl;
    } else {
        log_message("TTF initialized successfully");
        std::cerr << "TTF initialized successfully" << std::endl;
    }
    ...
    // Initialize simulation
    {
        ...
        std::cerr << "Simulation setup completed" << std::endl;
    }

    // Initialize menu
    Menu menu;
    bool menu_initialized = false;
    if (init_ttf()) { // Only try menu if TTF initialized
        menu_initialized = menu.init(window, sim);
        if (!menu_initialized) {
            log_message("Menu initialization failed, continuing without menu");
            std::cerr << "Menu initialization failed, continuing without menu" << std::endl;
        } else {
            log_message("Menu initialized successfully");
            std::cerr << "Menu initialized successfully" << std::endl;
        }
    } else {
        log_message("Skipping menu initialization due to TTF failure");
        std::cerr << "Skipping menu initialization due to TTF failure" << std::endl;
    }
    ...
    while (!quit) {
        while (SDL_PollEvent(&e)) {
            ...
            // Pass mouse events to menu if initialized
            if (menu_initialized && (e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN)) {
